...
Original file is located at
    https://colab.research.google.com/drive/1JrlGKaOlvMYXyoQle8-byICm5UbZN_0Y
...
class ParserConfiguration(object):
    ...
    def parse_step(self, transition):
        ...
        if "LA-" in transition:
            ...
            s2 = self.stack.pop()
            self.stack.push(s1)
            self.dependencies.add(s1, s2, label[1])
        
        # S2 --> S1
        if "RA-" in transition:
            s1 = self.stack.pop()
            s2 = self.stack[-1]
            self.dependencies.add(s2, s1, label[1])
...
def get_gold_action(stack, buffer, gold_dependencies):
    ...
    if len(stack) == 1 and stack[0].word == "<ROOT>":
        if not buffer:
            return "DONE"
        else:
            return "S"
    ...
    head_s1 = gold_dependencies.getArcToHead(s1).head
    
    head_s2 = gold_dependencies.getArcToHead(s2).head   
    
    if head_s2 == s1:
        label = gold_dependencies.getArcToHead(s2).label
        return "LA-"+label
        
    if head_s1 == s2:
        
        if all(gold_dependencies.getArcToHead(elem).head != s1 for elem in buffer):
            label = gold_dependencies.getArcToHead(s1).label
            return "RA-"+label
...
def generate_training_examples(sentence, gold_dependencies, vocab, feat_extract = lambda parser_config: []):
    ...
    while get_gold_action(parser.stack, parser.buffer, gold_dependencies) != "DONE":
        
        if get_gold_action(parser.stack, parser.buffer, gold_dependencies) == None:
            return []
        else:
            training_examples.append((feat_extract(parser), get_gold_action(parser.stack, parser.buffer, gold_dependencies)))
            parser.parse_step(get_gold_action(parser.stack, parser.buffer, gold_dependencies))
...
def get_top3_stack_features(parser_config):
    ...
    ### TODO ###
    length = len(parser_config.stack)
...
def get_top3_buffer_features(parser_config):
    ...
    length = len(parser_config.buffer)
...
def get_lc(parser_config, word):
    ...
    for arc in parser_config.dependencies:
        if arc.head == word:
            if arc.dependent.idx < arc.head.idx:             
                dic[arc] = arc.dependent.idx
...
def get_lc1_lc2_features(parser_config, i):
    ...
    if len(get_lc(parser_config, word)) >= 1:
        arc1 = get_lc(parser_config, word)[0]
        word_features[0] = arc1.dependent.word_id
        pos_features[0] = arc1.dependent.pos_id
        label_features[0] = arc1.label_id

    if len(get_lc(parser_config, word)) >= 2:
        arc2 = get_lc(parser_config, word)[1]
        word_features[1] = arc2.dependent.word_id
        pos_features[1] = arc2.dependent.pos_id
        label_features[1] = arc2.label_id
...
def get_rc1_rc2_features(parser_config, i):
    ...
    if len(get_rc(parser_config, word)) >= 1:
        arc1 = get_rc(parser_config, word)[0]
        word_features[0] = arc1.dependent.word_id
        pos_features[0] = arc1.dependent.pos_id
        label_features[0] = arc1.label_id

    if len(get_rc(parser_config, word)) >= 2:
        arc2 = get_rc(parser_config, word)[1]
        word_features[1] = arc2.dependent.word_id
        pos_features[1] = arc2.dependent.pos_id
        label_features[1] = arc2.label_id  
...
def get_llc_rrc_features(parser_config, i):
    ...
    if len(get_lc(parser_config, word)) >= 1:
        lc1_si = get_lc(parser_config, word)[0].dependent
        
        if len(get_lc(parser_config, lc1_si)) >= 1:
            lc1_lc1_si = get_lc(parser_config, lc1_si)[0]
            word_features[0] = lc1_lc1_si.dependent.word_id
            pos_features[0] = lc1_lc1_si.dependent.pos_id
            label_features[0] = lc1_lc1_si.label_id
    
    
    if len(get_rc(parser_config, word)) >= 1:
        rc1_si = get_rc(parser_config, word)[0].dependent
        
        if len(get_rc(parser_config, rc1_si)) >= 1:
            rc1_rc1_si = get_rc(parser_config, rc1_si)[0]
            word_features[1] = rc1_rc1_si.dependent.word_id
            pos_features[1] = rc1_rc1_si.dependent.pos_id
            label_features[1] = rc1_rc1_si.label_id
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        Pseudocode:
            ...
            (3) Apply dropout
            (4) Pass the result through the final linear layer and return its output 
                    - Output size: [batch_size, n_classes]
        '''
        
        # word embeddings
        e1 = self.embed(x)
        e2 = e1.view(x.shape[0], self.n_features * self.embed_size)
        
        # ReLU with Linear Layer
        c1 = F.relu(self.fc1(e2))
...
def select_best_legal_action(parser_configs, predictions, n_labels):
    ...
    bitmap = np.zeros((len(parser_configs), 2 * n_labels + 1))
    for i, cfg in enumerate(parser_configs): 

      if (len(cfg.stack) >= 3):
        bitmap[i][0:n_labels-1] = 999999999
      
      if (len(cfg.stack) >= 2):
        bitmap[i][n_labels: 2*n_labels-1] = 999999999

      if (len(cfg.buffer) != 0): 
        bitmap[i][-1] = 999999999

    added_preds = np.add(predictions, bitmap)
   
    preds = np.argmax(added_preds, axis = 1) 
    return preds
